function Board(ctx) {
  ...
  this.pickRandomDone = function () {
    ...
    for (var k = 0; k < numOfDotsAtStart; k++) {
      ...
      do {
        ...
        i === Math.floor(self.numOfBlocks / 2) &&
        j === Math.floor(self.numOfBlocks / 2)
      );
    ...
    for (var i = 0; i < self.numOfBlocks; i++) {
      for (var j = 0; j < self.numOfBlocks; j++) {
        ...
        if (self.grid[i][j].fill === true) {
          self.ctx.fillStyle = "rgb(139,0,0)";
        } else {
          self.ctx.fillStyle = "rgb(0,191,255)";
        }
        self.ctx.arc(
          self.grid[i][j].getPosition().x,
          self.grid[i][j].getPosition().y,
          self.radius,
          self.startAngle,
          self.endAngle,
          self.clockwise
  ...
  this.updateCircle = function (position) {
    ...
    for (var i = 0; i < self.numOfBlocks; i++) {
      for (var j = 0; j < self.numOfBlocks; j++) {
        if (
          self.grid[i][j].isInPosition(position) &&
          !self.grid[i][j].fill &&
          !self.grid[i][j].hasCat
        ) {
          self.grid[i][j].updateColor();
          self.draw();
          circleUpdated = true;
  ...
  this.findAvailablePositions = function () {
    ...
    if (
      ...
      currentPosition.row === self.numOfBlocks - 1 ||
      currentPosition.col === 0 ||
      currentPosition.col === self.numOfBlocks - 1
    ) {
  ...
  this.determineCatDirection = function () {
    ...
    if (shortestPaths.length === 0) {
      ...
      var block =
        shortestPaths[Math.floor(Math.random() * shortestPaths.length)][1];
