...
function Circle(x, y, r) {
  ...
  this.isInPosition = function (position) {
    var isCorrectWidth =
      self.x - self.radius + 3 < position.x &&
      position.x < self.x + self.radius - 3;
    var isCorrectHeight =
      self.y - self.radius + 3 < position.y &&
      position.y < self.y + self.radius - 3;
...
function Game(ctx) {
  ...
  this.winGame = function () {
    document.getElementById("panelTitle").innerHTML = "Hai vinto!";
    $("#panel").fadeIn(function () {});
    setTimeout(run, 2000);
  };
  this.loseGame = function () {
    document.getElementById("panelTitle").innerHTML = "Hai perso";
    $("#panel").fadeIn(function () {});
    setTimeout(run, 2000);
  };
}
...
function startUp() {
  if (checkCanvas()) {
    ...
    alert("Tu navegador es demasido antiguo.");
  }
}
